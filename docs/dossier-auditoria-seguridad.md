...
| 2026-07-22 | **Cierre de H01** (intervención sobre axioma, con backup + rollback preparado): `pg_hba.conf` acotado a `127.0.0.1/32`/`::1/128` con `scram-sha-256`, regla `0.0.0.0/0 md5` eliminada; `password_encryption = scram-sha-256` + re-hash de todos los roles (0 con hash md5). Aplicado con `pg_reload_conf()` (sin restart) y verificado en el server: apps conectando por loopback, 0 auth failures. | **H01 → ✅ cerrado**. Estado consolidado: **10 cerrados · 2 parciales (H04, H06) · 1 bloqueado (H09) · 1 pendiente (H11)**. Residual menor aceptado: 2 reglas de `mediflow_db` conservan método `md5` (hash del rol ya scram, acceso solo loopback). Secciones tocadas: §1 (resumen + tabla de madurez), §5 (CIS 4), §7 (consolidado + fila H01). |
