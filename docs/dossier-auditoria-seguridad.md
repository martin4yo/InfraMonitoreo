...
**no es técnica sino de gobierno**: al 2026-07-23 se redactaron los primeros 5 de 10 documentos formales
(G1–G5: política de seguridad, roles/RACI, gestión de riesgos, gestión de vulnerabilidades y respuesta a
incidentes), pendientes de firma/aprobación; los otros 5 (G6–G10) siguen ausentes, y quedan hallazgos
técnicos de defensa en profundidad (apps Next.js aún en `0.0.0.0`,
...
|---|---|---|
| Seguridad / Hardening | 🟡 Amarillo | Críticos cerrados en los 5 (SSH sin root/pass, ufw default-deny, token R2 rotado) y respaldos con credenciales purgados (H03, 07-19). El último 🔴 de config cerró el 07-22: pg_hba de axioma acotado a loopback + scram (H01). Pendientes de defensa en profundidad: 5 apps Next.js aún en `0.0.0.0` (H04, bloqueado por un patrón del framework), propiedad de archivos de mini/axioma (H06, requiere ventana). |
...
| **Gobierno / Documentación formal** | 🔴 Rojo | Al 2026-07-23, 5 de 10 documentos redactados (G1–G5: política de seguridad, matriz de roles/RACI, registro de riesgos, gestión de vulnerabilidades/parches y plan de respuesta a incidentes), pendientes de firma/aprobación; G6–G10 aún ausentes. Ver §8. |
...
- **Gobierno formal**: 5 de 10 documentos redactados el 2026-07-23 (G1–G5, pendientes de firma/aprobación); G6–G10 **aún no existen** (declarados como gaps en §8).
...
## 8. Gaps de gobierno (documentos que un auditor pediría; 5 de 10 redactados al 2026-07-23, pendientes de firma/aprobación)
...
- [x] **G3 — Gestión de riesgos.** ✅ **redactado (2026-07-23)** en [`gobierno/g3-registro-riesgos.md`](./gobierno/g3-registro-riesgos.md): registro formal con metodología P×I, **12 riesgos operativos/gobierno (R01–R12) + 4 escenarios de desastre (RD-A…D)**, cada uno con probabilidad, impacto, nivel, tratamiento y owner. Consolida los hallazgos abiertos (H04, H06, H09, H11), los residuales aceptados (creds R2 en `.conf`, password débil de mediflow, creds AWS sin uso), el bus factor de ejecución (residual de G2) y los escenarios del DRP. **Prioritarios 🔴:** R08 (bus factor de ejecución) y RD-A (corrupción de datos). **Aceptados pendientes de firma de la AN:** R03, R05, R12.
...
- [ ] **G7 — Gestión formal de secretos y rotación.** SOPS+age está implementado, pero no hay **política de rotación** de credenciales (el token R2 se rotó reactivamente por exposición, no por calendario) ni inventario formal de secretos con owner y frecuencia de rotación. Candidatos ya identificados que esperan esa política: password de `mediflowuser` **corto/débil (8 caracteres)** — anotado durante el cierre de H01 (07-22) — y las credenciales AWS del `.env` de hub, declaradas para un SDK **nunca importado** (revocar, anotado en H13).
