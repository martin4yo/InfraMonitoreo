...
| **S19** | **`google-credentials.json`** — service account de Google (Document AI), proyecto `axioma-448223` | `/var/www/parse/backend/google-credentials.json` + **`infra-secrets/creds/axioma/`** (SOPS) | RT | 🔴 Crítica | — | **Anual** | ✅ **RESPALDADA 2026-08-12.** Descubierta sin respaldo en el simulacro —su única copia estaba en axioma— y cifrada el mismo día. Verificado: 11 campos cifrados, roundtrip a JSON válido con la clave PEM íntegra. Regla nueva en `.sops.yaml` para `creds/*.json` |
| **S20** | **Backup de adjuntos a R2 (restic)** — `RESTIC_PASSWORD` (cifra el repositorio) + API token de R2 *Object Read & Write* solo sobre el bucket `axiomacloud-adjuntos`, con filtro de IP | `/etc/restic/alvera-adjuntos.env` (axioma, `600 root`) + **`infra-secrets/env/restic/alvera-adjuntos.env`** (SOPS) | RT | 🔴 Crítica | — (creados 2026-09-15) | Token: **anual** · `RESTIC_PASSWORD`: **no rotar** (igual que S02: perderla hace ilegibles los snapshots; si hay que cambiarla, `restic key add` + `key remove`, nunca reemplazo directo) | ✅ **Vigente.** Token separado del de pgBackRest (S03) a propósito. Ver [backup-adjuntos.md](../backup-adjuntos.md) §5 — ⚠️ el filtro de IP usa las IPs de **salida** (axioma sale por IPv6) |
