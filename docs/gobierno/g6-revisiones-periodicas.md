...
- [ ] `sudoers` + `sudoers.d`: `NOPASSWD` **solo** para `axiomacloud` (regla explícita en `90-axiomacloud`); el grupo `%sudo` con password. *(Ampliado 2026-09-03 — R24.)*
- [ ] Membresías de **`sudo`, `lxd` y `docker`** (`getent group`): solo cuentas humanas de operación; **ninguna `<app>app`**. *(Ampliado 2026-09-03: `axioapp` estaba en `sudo` de dev-1.)*
- [ ] `passwd -Sa`: cuentas con password activa (`P`) enumeradas y justificadas; las `<app>app` con password figuran como residual aceptado en G3 R24. *(Ampliado 2026-09-03.)*
