...
| **R21** | 🔴 **El proveedor filtra los puertos 80/443 hacia axioma-drp: el DRP es inejecutable en su último paso** | Simulacro DRP 2026-08-12 · [runbook §8.1](../drp-recuperacion-axioma-en-drp.md) | **Materializado** | **A** | 🔴 **Alto** | Mitigar | **El servidor está impecable y aun así no sirve.** nginx escucha en `0.0.0.0:80`, `iptables` tiene los `ACCEPT` para 80 y 443, y desde drp `127.0.0.1:80` devuelve **200**. Pero **desde el propio drp su IP pública devuelve 000**, y desde afuera 80 y 443 están **filtrados** mientras el 22 abre normalmente. En axioma, el mismo test sobre el 80 abre. **El bloqueo está aguas arriba del servidor**, en la red del proveedor. **Consecuencia para el DRP:** se puede ejecutar el runbook completo —restaurar la base, levantar las apps, mover el DNS— y **el servicio sigue caído**, con `ufw status` informando que todo está bien y las apps respondiendo perfecto en loopback. Es el peor modo de falla posible: **invisible desde el servidor y solo detectable desde afuera**. *Acción:* solicitar a **baehost** —proveedor de axioma-drp, **distinto** del de producción— la habilitación de 80/443 entrantes para `170.78.75.249`, o revisar si su panel tiene un firewall de red separado del del sistema operativo. **Mitigación provisional:** túnel SSH sobre el 22 (`scripts/80-drp-tunnel.sh`), que sirve para verificar pero **no** para dar servicio | RT | AN |
...
- **Total:** **21** riesgos operativos/gobierno + 4 escenarios de desastre. *(2026-08-12: **+R21**, el único que se descubrió **ejecutando** y no leyendo. 2026-08-09: +8 — R13 a R20,
...
- **🔴 Alto:** R08 (bus factor de ejecución), **R13** (posibles datos productivos 🔴 en dev-1),
  **R14** (el blocklist del db-agent no protege datos clínicos — **verificado**), **R17** (repo de secretos
  ...
  proveedor a dev-1 — **uso confirmado**), **R21** (80/443 filtrados hacia drp — **el DRP no puede completarse**),
  RD-A (corrupción de datos).
