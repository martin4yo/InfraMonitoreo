...
| **R23** | **Controles de host que no sobreviven a un reboot** — un control aplicado en caliente (ufw, fail2ban, drop-ins) puede no estar habilitado al arranque, y **ningún server salvo drp tiene auto-chequeo** | Spot-check 2026-09-03 · [hardening §dev-1](../hardening.md) | M | **A** | 🟡 **Medio** *(🔴 → 🟡 el mismo día: selfcheck en 5/5)* | Mitigar | **Materializado en dev-1:** rebooteó el 2026-08-14 (sin registro) con `ufw.service` `disabled` → **20 días sin firewall de host** en el repo host de todos los backups, tapado únicamente por el firewall de red del proveedor. Re-habilitado el 2026-09-03 con unit `enabled`, verificado desde afuera + `pgbackrest check` 4/4. **Sigue abierto** porque la causa es estructural: (a) ✅ `hardening-selfcheck.sh` **desplegado en los 5 el 2026-09-03** (era solo drp) — de acá el 🟡; la primera corrida ya expuso **fail2ban caído en axioma y ausente en axiodemo**; (b) ✅ fail2ban **habilitado en axioma e instalado en axiodemo el 2026-09-03** (estaba inactivo/ausente, el dossier decía "en los 5"); selfcheck `ok=1` en 5/5; (c) los cierres de G10 deben verificar `systemctl is-enabled` y no solo el estado vivo (regla R7, ampliar). Cierra en 🟢 con selfcheck en 5/5 y un reboot controlado (C11) que confirme que todo vuelve | RT | — |
| **R24** | **Cuentas privilegiadas fuera de inventario** — usuarios de aplicación con password activa o en grupos con sudo, y un usuario humano no documentado | Spot-check 2026-09-03 · [hardening §13](../hardening.md) | M | M | 🟡 Medio | Mitigar | Encontrado: `axioapp` en el grupo `sudo` de dev-1 (con `NOPASSWD` para todo el grupo → root sin password para una cuenta de app); `ubuntu` con `NOPASSWD` ×6 y en `lxd` en drp; **`hervierh`** con password en axioma sin figurar en G2 ni en ningún doc; `miniapp`/`alveraapp` con password. ✅ **Mitigado el mismo día:** `NOPASSWD` acotado a `axiomacloud` en axioma/clubix/dev-1 y `ubuntu` sin privilegios en drp. **`hervierh` identificado (2026-09-03, RT):** persona del equipo; la cuenta se creó el 06-27 para que subiera contenido a una carpeta puntual del sitio de axioma. Nunca la usó (0 logins, sin llave). **Decisión del RT: se deja como está.** Si se activa, hacerlo como SFTP enjaulado con llave (ver hardening §13), no con password. **Abierto:** quitar password a las cuentas `<app>app` (el RT decidió no tocarlas por ahora), sacar `axioapp` del grupo `sudo`, y que C4 incluya `passwd -Sa` + membresías de `sudo`/`lxd`/`docker` — hoy solo mira `sudoers` | RT | — |
...
- **Total:** **24** riesgos operativos/gobierno (**4 cerrados**: R18, R19, R21, R22) + 4 escenarios de desastre. *(2026-09-03: **+R23**, encontrado en un spot-check contra los servers: dev-1 20 días sin firewall de host.)* *(2026-08-12: **+R21**, el único que se descubrió **ejecutando** y no leyendo. 2026-08-09: +8 — R13 a R20,
...
- **🟡 Medio:** R02, R06, R09, R10, R11, R12, **R16**, **R17**, **R23**, **R24**, RD-B, RD-C, RD-D. *(R19 cerrado el 2026-09-03.)*
