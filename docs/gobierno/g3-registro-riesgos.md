...
| **R22** | **Backups de configuración de `sshd` con `PermitRootLogin yes` dentro de `sshd_config.d`** | Colateral del cierre de R18 (2026-08-13) | B | **A** | 🟡 Medio | Evitar | En **axioma, clubix y dev-1** quedaron archivos `custom.conf.bak-20260717-*` (axiodemo y **axioma-drp están limpios**) **dentro de `/etc/ssh/sshd_config.d/`**, con la configuración previa al hardening: **`PermitRootLogin yes`**. **Hoy no se aplican** —el `Include` es `*.conf` y esos nombres terminan en dígitos— pero es **el mismo patrón que R18**: un archivo con configuración insegura esperando dentro del directorio que `sshd` lee. Basta renombrarlo *«para restaurar»* y vuelve el login de root, sobre servidores donde hay **~43 llaves del proveedor en `/root/.ssh/authorized_keys`** (R19) que hoy están inertes **solo** por esa directiva. *Acción:* moverlos fuera de `sshd_config.d` —a `/root/`, como se hizo con el de R18— en vez de dejarlos ahí | RT | — |
...
- **Total:** **22** riesgos operativos/gobierno (**2 cerrados**: R18, R21) + 4 escenarios de desastre. *(2026-08-12: **+R21**, el único que se descubrió **ejecutando** y no leyendo. 2026-08-09: +8 — R13 a R20,
...
- **🟡 Medio:** R02, R06, R09, R10, R11, R12, **R16**, **R17**, **R19**, **R22**, RD-B, RD-C, RD-D.
