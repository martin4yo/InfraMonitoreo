...
| **R24** | **Cuentas privilegiadas fuera de inventario** — usuarios de aplicación con password activa o en grupos con sudo, y un usuario humano no documentado | Spot-check 2026-09-03 · [hardening §13](../hardening.md) | M | M | 🟡 Medio | Mitigar | Encontrado: `axioapp` en el grupo `sudo` de dev-1 (con `NOPASSWD` para todo el grupo → root sin password para una cuenta de app); `ubuntu` con `NOPASSWD` ×6 y en `lxd` en drp; **`hervierh`** con password en axioma sin figurar en G2 ni en ningún doc; `miniapp`/`alveraapp` con password. ✅ **Mitigado el mismo día:** `NOPASSWD` acotado a `axiomacloud` en axioma/clubix/dev-1 y `ubuntu` sin privilegios en drp. **Abierto:** identificar `hervierh` (¿quién es, sigue activo?), quitar password a las cuentas `<app>app`, sacar `axioapp` del grupo `sudo`, y que C4 incluya `passwd -Sa` + membresías de `sudo`/`lxd`/`docker` — hoy solo mira `sudoers` | RT | — |
