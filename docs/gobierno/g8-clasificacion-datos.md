...
| **A10** | ~~Reducir el acceso del proveedor: purgar las 48 llaves `@donweb.com` de `/home/axiomacloud/.ssh/authorized_keys` en dev-1~~ ✅ **purgadas el 2026-09-03** (y las 43 de `root`, que quedó sin llaves); acceso propio verificado con sesión nueva, backups `.bak` en `/root`. **Queda la segunda mitad**: **sacar los datos productivos de dev-1** — hasta entonces, cualquier llave que el proveedor vuelva a inyectar (como pasó en la reinstalación de drp) reabre el acceso a datos reales. Detectable por C4 y por el inventario generado | 🟡 **Parcial** | RT (coordinar con el proveedor) |
